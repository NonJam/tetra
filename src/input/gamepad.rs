...
pub fn is_gamepad_connected(ctx: &InputContext, gamepad_id: usize) -> bool {
...
pub fn get_gamepad_name(ctx: &Context, gamepad_id: usize) -> Option<String> {
    get_gamepad(&ctx.input, gamepad_id)
        .map(|g| g.platform_id)
...
pub fn is_gamepad_button_down(ctx: &InputContext, gamepad_id: usize, button: GamepadButton) -> bool {
...
pub fn is_gamepad_button_up(ctx: &InputContext, gamepad_id: usize, button: GamepadButton) -> bool {
    if let Some(pad) = get_gamepad(ctx, gamepad_id) {
        !pad.buttons_down.contains(&button)
    } else {
        true
...
///
/// If the gamepad is disconnected, this will always return `false`.
pub fn is_gamepad_button_pressed(ctx: &InputContext, gamepad_id: usize, button: GamepadButton) -> bool {
    if let Some(pad) = get_gamepad(ctx, gamepad_id) {
...
///
/// If the gamepad is disconnected, this will always return `false`.
pub fn is_gamepad_button_released(ctx: &InputContext, gamepad_id: usize, button: GamepadButton) -> bool {
    if let Some(pad) = get_gamepad(ctx, gamepad_id) {
...
pub fn get_gamepad_axis_position(ctx: &InputContext, gamepad_id: usize, axis: GamepadAxis) -> f32 {
    if let Some(pad) = get_gamepad(ctx, gamepad_id) {
...
pub fn is_gamepad_vibration_supported(ctx: &Context, gamepad_id: usize) -> bool {
    if let Some(pad) = get_gamepad(&ctx.input, gamepad_id) {
...
pub fn set_gamepad_vibration(ctx: &mut Context, gamepad_id: usize, strength: f32) {
    if let Some(platform_id) = get_gamepad(&ctx.input, gamepad_id).map(|g| g.platform_id) {
...
pub(crate) fn add_gamepad(ctx: &mut InputContext, platform_id: u32) -> usize {
    for (i, slot) in ctx.pads.iter_mut().enumerate() {
        if slot.is_none() {
    ...
    let i = ctx.pads.len();
    ctx.pads.push(Some(GamepadState::new(platform_id)));
    i
}

pub(crate) fn remove_gamepad(ctx: &mut InputContext, gamepad_id: usize) {
    ctx.pads[gamepad_id] = None;
}

pub(crate) fn get_gamepad(ctx: &InputContext, gamepad_id: usize) -> Option<&GamepadState> {
    if let Some(Some(pad)) = ctx.pads.get(gamepad_id) {
        Some(pad)
    } else {
        None
    }
...
pub(crate) fn get_gamepad_mut(ctx: &mut InputContext, gamepad_id: usize) -> Option<&mut GamepadState> {
    if let Some(Some(pad)) = ctx.pads.get_mut(gamepad_id) {
        Some(pad)
    } else {
        None
    }
